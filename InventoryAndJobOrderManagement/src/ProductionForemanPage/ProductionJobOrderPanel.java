...
public class ProductionJobOrderPanel extends JPanel {
    ...
    public ProductionJobOrderPanel() {
        setLayout(null);
        setBackground(new Color(245, 245, 245));

        // ===== HEADER =====
        JLabel title = new JLabel("PRODUCTION FOREMAN DEPARTMENT - JOB ORDERS");
    ...
    // Replace the createQuickStats() method in ProductionJobOrderPanel.java with this:

    private void createQuickStats() {
        // Active Jobs = In Progress status
        int activeJobs = JobOrderDao.getJobOrdersByDepartment("Electrical", "In Progress");

        // Pending Jobs = Pending status
        int pendingJobs = JobOrderDao.getJobOrdersByDepartment("Electrical", "Pending");

        // Completed Jobs = Completed stage (not status)
        int completedJobs = JobOrderDao.getJobOrdersByDepartmentAndStage("Electrical", "Completed");

        // Total = count all job orders for Mechanical department
        int totalJobs = activeJobs + pendingJobs + completedJobs +
                JobOrderDao.getJobOrdersByDepartment("Electrical", "On Hold");
    ...
    private void createJobOrderTable() {
        ...
        jobOrderTable.getColumnModel().getColumn(1).setPreferredWidth(200);
        jobOrderTable.getColumnModel().getColumn(2).setPreferredWidth(150);
        jobOrderTable.getColumnModel().getColumn(3).setPreferredWidth(100);
        jobOrderTable.getColumnModel().getColumn(4).setPreferredWidth(120);
        jobOrderTable.getColumnModel().getColumn(5).setPreferredWidth(100);
        jobOrderTable.getColumnModel().getColumn(6).setPreferredWidth(200);
    ...
    // Replace the loadJobOrders() method in ProductionJobOrderPanel.java with this:

    private void loadJobOrders() {
        try {
            // Get job orders for Mechanical department only
            List<Object[]> rows = JobOrderDao.getJobOrdersByDepartment("Electrical");
            tableModel.setRowCount(0);

            for (Object[] row : rows) {
                // DAO returns: [job_id, client_name, employee_assigned, deadline, stage, status, materials, notes]
                // Table expects: [Job ID, Client Name, Employee Assigned, Deadline, Stage, Status, Progress Notes]
                ...
                tableModel.addRow(new Object[]{
                        ...
                        row[4],     // Stage
                        row[5],     // Status
                        row[7]      // Progress Notes (skip materials at index 6)
                });
            }
    ...
    private void viewJobOrderDetails() {
        ...
        ProductionJobOrderDetailsDialog dialog = new ProductionJobOrderDetailsDialog(
